function [pts_out,xy_out,cluster_sizes,clusters] = meanShiftSegmentOld(X,bandwidth,options)
...
% Vol. 24, No. 5, May 2002.
    ...
    elseif options.mode == 'M'
        disp('Computing mean shift only');
    elseif options.mode == 'C'
        disp('Computing clusters and sizes -- assuming X are already shifted')
        options.maxLoops = 0;
    elseif options.mode == 'S'
        disp('Computing cluster sizes only -- assuming X are already shifted')
        options.maxLoops = 0;
    elseif options.mode == 'L'
        options.maxLoops = 0;
        if isempty(options.csIn)
            disp('You need to pass in cluster sizes as options.csIn when only computing clusters');
        else
            disp('Computing clusters only -- assuming X are already shifted and cs passed in')
        end
    else
        disp('Valid modes are [A]ll, [M]ean shift only, C[L]usters only, Cluster [S]ize only, or [C] for cluster size and clusters');
    end
    ...
    if (options.mode == 'L') || (options.mode == 'M')
        cluster_sizes = options.csIn;
    else
    ...
    if (options.mode == 'S') || (options.mode == 'M')
        clusters = currX;
    else
...
function [cluster_sizes] = computeClusterSizes(currX,bandwidth)
    ...
    for r=1:l
        ...
        for c=1:w
            ...
            distance = sum((currX(startY:endY,startX:endX,end-1:end) - currX(r,c,end-1:end)).^2,3) <= bandwidth(2);
