...
% options.debug = 1;
% options.mode = 'M';
% [pts_out,xy_out,~,~,~] = meanShiftSegment7(data,bw,options);
% msPts = cat(3,pts_out,xy_out);
load('bc.mat');
load('cs.mat');
options.mode = 'C';
options.cs = cluster_sizes;
[~,~,~,clusters,~] = meanShiftSegment8(data,bw,options);
options.smooth = 1;
[~,~,~,clusters2,~] = meanShiftSegment8(data,bw,options);
disp('l');
% tic
...
bw = [8,8,10];
options.mode = 'C';
[~,~,~,clusters2,~] = meanShiftSegment7(msPts,bw,options);
bw = [8,8,50];
[~,~,~,clusters3,~] = meanShiftSegment7(msPts,bw,options);
figure;
subplot(1,3,1);
% imshow(im);
% subplot(1,3,2);
